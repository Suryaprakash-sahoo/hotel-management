...
function ComboboxClear({ className, ...props }: ComboboxPrimitive.Clear.Props) {
  return (
    <ComboboxPrimitive.Clear data-slot="combobox-clear" className={cn("pointer-events-auto", className)} {...props} render={<InputGroupButton variant="ghost" size="icon-xs" className="pointer-events-auto cursor-pointer"><XIcon /></InputGroupButton>} />
  )
}
...
function ComboboxInput({
  ...
  return (
    <InputGroup className={cn("w-auto pointer-events-auto", className)}>
      <ComboboxPrimitive.Input
        render={<InputGroupInput disabled={disabled} className="pointer-events-auto cursor-text" />}
        {...props}
      />
      <InputGroupAddon align="inline-end" className="pointer-events-auto">
        {showTrigger && (
          <InputGroupButton
            ...
            className="group-has-data-[slot=combobox-clear]/input-group:hidden data-pressed:bg-transparent pointer-events-auto cursor-pointer"
            disabled={disabled}
          />
        )}
...
function ComboboxList({ className, ...props }: ComboboxPrimitive.List.Props) {
  return (
    <ComboboxPrimitive.List
      ...
      className={cn(
        "no-scrollbar max-h-[min(calc(--spacing(72)---spacing(9)),calc(var(--available-height)---spacing(9)))] scroll-py-1 p-1 data-empty:p-0 overflow-y-auto overscroll-contain pointer-events-auto",
        className
      )}
      {...props}
    />
...
function ComboboxItem({
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "data-highlighted:bg-accent data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground gap-2 rounded-md py-1 pr-8 pl-1.5 text-sm [&_svg:not([class*='size-'])]:size-4 relative flex w-full cursor-pointer items-center outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:shrink-0 pointer-events-auto",
        className
      )}
      {...props}
    >
      {children}
      <ComboboxPrimitive.ItemIndicator render={<span className="absolute right-2 flex size-4 items-center justify-center"><CheckIcon /></span>} />
    </ComboboxPrimitive.Item>
  )
}
